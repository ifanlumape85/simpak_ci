...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('pengaturan_instansi/ajax_list')?>",
    ...
    $('#btn-filter').click(function(){ //button filter event click
        reload_pengaturan_instansi_table();  //just reload table
    });
...
function add_pengaturan_instansi()
{
    save_method = 'add';
    ...
    $('.modal-title').text('Tambah Pengaturan Instansi'); // Set Title to Bootstrap modal title

}

function edit_pengaturan_instansi(id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('pengaturan_instansi/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
         $('[name="id"]').val(data.id_pengaturan_instansi);
         ...
         $('[name="tanggal"]').val(data.tanggal);
            ...
            $('.modal-title').text('Edit Pengaturan Instansi'); // Set title to Bootstrap modal title           
        },
...
function reload_pengaturan_instansi_table()
{
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('pengaturan_instansi/ajax_add')?>";
    } else {
        url = "<?php echo site_url('pengaturan_instansi/ajax_update')?>";
    }
    ...
    $.ajax({        
        ...
        success: function(data)
        {
            ...
            {
                reload_pengaturan_instansi_table();
                if (save_method== 'add')
                {
				    $('#form')[0].reset(); // reset form on modals
				    $('.form-group').removeClass('has-error'); // clear error class
...
function delete_pengaturan_instansi(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('pengaturan_instansi/ajax_delete')?>/"+id,
            type: "POST",
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                ...
                url: "<?php echo site_url('pengaturan_instansi/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
...
<div class="modal fade" id="modal_form" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Pengaturan Instansi Form</h3>
            </div>
    ...
    <div class="form-group">
       ...
       <div class="col-md-9">
           <input placeholder="DD-MM-YYYY" type="text" name="tanggal" id="datepicker" class="form-control" />
           <span class="help-block"></span>
       </div>
   </div>
