...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('kegiatan_bulanan/ajax_list')?>",
            "type": "POST", "data":function(data){
                ...
                data.id_kegiatan_tahunan = $('select[name=id_kegiatan_tahunan2]').val();
            }
        },
    ...
    $('[name=tahun2]').keyup(function(){
        ...
        if (ltahun > 2)
        {
            $.ajax({
                ...
                type: "POST",
                data:{tahun:tahun,id_pegawai:id_pegawai,csrf_test_name:'<?=$this->security->get_csrf_hash()?>'},
                dataType: "text",
                success: function(data)
                {
...
function edit_kegiatan_bulanan(id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    if (user_level_id > 1)
    ...
    $.ajax({
        url : "<?php echo site_url('kegiatan_bulanan/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			$('[name="id"]').val(data.id_kegiatan_bulanan);
			$('[name="id_kegiatan_tahunan"]').val(data.id_kegiatan_tahunan);
            ...
            var id_kegiatan_tahunan = data.id_kegiatan_tahunan;
            ...
            $.ajax({
                ...
                {
                    $('select[name=id_kegiatan_tahunan]').html(data);
                    $('select[name=id_kegiatan_tahunan]').val(id_kegiatan_tahunan);       
                },
                error: function (jqXHR, textStatus, errorThrown)
                {
                    // alert('Error get data from ajax');
            ...
            $('.modal-title').text('Edit Kegiatan Bulanan'); // Set title to Bootstrap modal title           
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Error get data from ajax');
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('kegiatan_bulanan/ajax_add')?>";
    } else {
        url = "<?php echo site_url('kegiatan_bulanan/ajax_update')?>";
    }
...
function delete_kegiatan_bulanan(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('kegiatan_bulanan/ajax_delete')?>/"+id,
            type: "POST",
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                ...
                url: "<?php echo site_url('kegiatan_bulanan/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
