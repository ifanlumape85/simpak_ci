...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('kegiatan_harian/ajax_list')?>",
    ...
    $('select[name=id_instansi2]').change(function() {
        ...
        $.ajax({
            ...
            {
                alert('Error get instansi 2 from ajax');
            }
        });    
    });
    ...
    $('select[name=id_pegawai]').change(function() {
        var id = $('select[name=id_pegawai]').val();
        
        $.ajax({
            url : "<?php echo site_url('verifikator/option_verifikator2/')?>/" + id,
            type: "POST",
            data: {id_pegawai:id, csrf_test_name:'<?=$this->security->get_csrf_hash()?>'},
            dataType: "text",
            success: function(data)
            {
        ...
        $.ajax({
            url : "<?php echo site_url('verifikator/get_verifikator/')?>/" + id,
            type: "POST",
    ...
    $.ajax({
        url : "<?php echo site_url('verifikator/option_instansi_verifikator/')?>/" + instansi_id,
        type: "POST",
        data: {id_instansi:instansi_id,csrf_test_name:'<?=$this->security->get_csrf_hash()?>'},
        dataType: "text",
        success: function(data)
        {
            $('#id_verifikator').html(data);       
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
            alert('Error get instansi verifikator from ajax');
        }
    });
});
...
function add_kegiatan_harian()
{
    ...
    if (user_level_id > 1)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                if (user_level_id==5)
                {
                    ...
                    $.ajax({
                        url : "<?php echo site_url('verifikator/get_verifikator/')?>/" + pegawai_id,
                        type: "POST",
                        data: {id:id, csrf_test_name:'<?=$this->security->get_csrf_hash()?>'},
                        dataType: "json",
                        success: function(data)
        ...
        $.ajax({
            url : "<?php echo site_url('presensi/cek_presensi_ajax/')?>/" + pegawai_id,
            type: "POST",
