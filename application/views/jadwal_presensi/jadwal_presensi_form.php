...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('jadwal_presensi/ajax_list')?>",
    ...
    $('#btn-filter').click(function(){ //button filter event click
        reload_jadwal_presensi_table();  //just reload table
    });
...
function add_jadwal_presensi()
{
    ...
    $('.modal-title').text('Tambah Jadwal Presensi'); // Set Title to Bootstrap modal title

}

function edit_jadwal_presensi(id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('jadwal_presensi/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
         $('[name="id"]').val(data.id_jadwal_presensi);
         ...
         $('[name="akhir"]').val(data.akhir);
            ...
            $('.modal-title').text('Edit Jadwal Presensi'); // Set title to Bootstrap modal title           
        },
...
function reload_jadwal_presensi_table()
{
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('jadwal_presensi/ajax_add')?>";
    } else {
        url = "<?php echo site_url('jadwal_presensi/ajax_update')?>";
    }
...
function delete_jadwal_presensi(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('jadwal_presensi/ajax_delete')?>/"+id,
            type: "POST",
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                ...
                url: "<?php echo site_url('jadwal_presensi/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Jadwal Presensi Form</h3>
            </div>
            <div class="modal-body form">
                ...
                <div class="form-body">
                 ...
                 <div class="form-group">
                   ...
                   <div class="col-md-9">
                       <input placeholder="DD-MM-YYYY" type="text" name="tgl_presensi" id="datepicker" class="form-control" />
                       <span class="help-block"></span>
                   </div>
               </div>
