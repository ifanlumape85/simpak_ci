...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('pengaturan_jadwal/ajax_list')?>",
    ...
    $('#btn-filter').click(function(){ //button filter event click
        reload_pengaturan_jadwal_table();  //just reload table
    });
...
function add_pengaturan_jadwal()
{
    save_method = 'add';
    ...
    $('.modal-title').text('Tambah Pengaturan Jadwal'); // Set Title to Bootstrap modal title

}

function edit_pengaturan_jadwal(id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
        url : "<?php echo site_url('pengaturan_jadwal/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			$('[name="id"]').val(data.id_pengaturan_jadwal);
            ...
            $('.modal-title').text('Edit Pengaturan Jadwal'); // Set title to Bootstrap modal title           
        },
...
function reload_pengaturan_jadwal_table()
{
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('pengaturan_jadwal/ajax_add')?>";
    } else {
        url = "<?php echo site_url('pengaturan_jadwal/ajax_update')?>";
    }
    ...
    $.ajax({        
        ...
        success: function(data)
        {
            ...
            {
                reload_pengaturan_jadwal_table();
                if (save_method== 'add')
                {
				    $('#form')[0].reset(); // reset form on modals
				    $('.form-group').removeClass('has-error'); // clear error class
...
function delete_pengaturan_jadwal(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('pengaturan_jadwal/ajax_delete')?>/"+id,
            type: "POST",
    ...
    if(list_id.length > 0)
    {
        if(confirm('Are you sure delete this '+list_id.length+' data?'))
        {
            $.ajax({
                ...
                url: "<?php echo site_url('pengaturan_jadwal/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data)
                {
                    if(data.status)
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h3 class="modal-title">Pengaturan Jadwal Form</h3>
            </div>
            <div class="modal-body form">
				...
				<div class="form-group">
					...
					<div class="col-md-9">
					<input placeholder="DD-MM-YYYY" type="text" name="tgl_presensi" class="form-control" />
					<span class="help-block"></span>
					</div>
				</div>
